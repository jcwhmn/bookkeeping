...
Define the storage and validation of icon, color, and comment metadata for
categories. These visual properties enhance the UI by allowing users to
customize how each category appears in lists, charts, and selection
controls. The system SHALL validate color format strictly (6-character
hex with `#` prefix) and SHALL treat all three fields as optional.
