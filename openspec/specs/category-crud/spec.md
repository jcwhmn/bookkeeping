...
Define RESTful CRUD operations for transaction categories. Categories are
user-owned resources used to classify transactions as INCOME, EXPENSE, or
TRANSFER. Each category has a name, type, optional icon/color/comment, and
sort order. The system SHALL provide endpoints for listing, searching,
creating, updating, hiding, and reordering categories, all scoped to the
authenticated user.
